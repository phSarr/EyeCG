...
// TODO display user info from DB + use flutter offline package
...
_saveData(String name, String phone, String contact1, String contact2,
    String contact3) async {
  SharedPreferences _prefs = await SharedPreferences.getInstance();
  _prefs.setString("name", name);
  _prefs.setString("phone", phone);
  _prefs.setString("contact1", contact1);
  if (contact2 != null) {
    _prefs.setString("contact2", contact2);
  }
  if (contact2 != null) {
    _prefs.setString("contact3", contact3);
  }
}
            ...
            children: [
              ...
              FormBuilderCheckboxGroup<String>(
                ...
                validator: (val) => /*val!.isEmpty? 'Required Field':*/
                    ((val!.contains('High blood pressure') &&
                            val.contains('Low blood pressure'))
              ...
              FormBuilderTextField(
                name: 'emergency contact1',
                controller: contact1Controller,
                // ignore: prefer_const_constructors
                decoration: InputDecoration(
                  labelText: 'Emergency Contact 1',
                  suffixIcon: const Icon(Icons.contact_phone_rounded),
                ),
                valueTransformer: (val) => val?.toString(),
                validator: FormBuilderValidators.compose([
                  FormBuilderValidators.match(
                      "[\\+]?[(]?[0-9]{3}[)]?[-\\s\\.]?[0-9]{3}[-\\s\\.]?[0-9]{4,6}\$",
                      errorText: 'Enter a Valid Number'),
                  FormBuilderValidators.required(errorText: 'Type in a Number')
                ]),
                keyboardType: TextInputType.number,
                textInputAction: TextInputAction.next,
              ),
...
Widget _resetFormButton(BuildContext context) {
  //TODO reset functionality not properly working
...
Future<void> _register(BuildContext context) async {
  ...
  BlocProvider.of<PhoneAuthCubit>(context)
      .submitPhoneNumber(phoneNumberController.text);
}
    ...
    listener: (context, state) {
      ...
      if (state is PhoneNumberSubmitted) {
        ...
        Navigator.of(context)
            .pushNamed(otpScreen, arguments: phoneNumberController.text);
      }
